...
public class BlockUnblock extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==search) {
			...
			if(userCredentials.getAccountStatus()==STATUS.OPEN) {
				int choice=JOptionPane.showConfirmDialog(null, "User account is open. Want to block it?");
				if(choice==0) {
					//yes
					userCredentials.setAccountStatus(STATUS.BLOCKED);
				}
			}else if(userCredentials.getAccountStatus()==STATUS.BLOCKED) {
				int choice=JOptionPane.showConfirmDialog(null, "User account is Blooked. Want to unblock it?");
				if(choice==0) {
					//yes
